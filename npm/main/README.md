...
context window 段在原本的 `百分比 · token 数` 之后，追加显示**最近一次 assistant 回合**的缓存命中率：

```
原版:  ⚡ 10.1% · 20.1k tokens
本版:  ⚡ 10.1% · 20.1k tokens · cache 89.6%
```

- 命中率定义：`cache_read /(input + cache_creation + cache_read)`，取自 transcript 中最近一条 assistant 消息的 `usage`。
- 为当前回合的瞬时值（与原有 context 百分比口径一致），非整会话累计。
