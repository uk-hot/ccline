...
### Changed
- **Second status line now shows *used* percentages** for the session and week, matching the numbers in Claude Code's `/usage` (previously it showed the remaining quota). Coloring is still driven by how much is left (green plenty → red nearly out), so e.g. `📊 Session 4% 🔄 17:20   Week 29% 🔄 6/21 23:00`.
